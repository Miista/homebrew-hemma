...
Authelia rules can never drift apart. The site block keeps the uniform gated shape above (no
per-path blocks), and each entry becomes a `bypass` rule in the generated
`hemma.access_control.generated.yml`, ahead of the service's access rule:
...
Two consequences: the exemptions only work once that generated file is **wired into Authelia**
(`hemma doctor` warns, listing the still-gated paths, until `X_AUTHELIA_CONFIG` loads it), and
bypassed paths **require the auth provider to be up** — every request still traverses the
